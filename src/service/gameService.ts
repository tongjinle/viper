...
export default class GameService {
  ...
  async createUser(userId: string, username: string): Promise<void> {
    await GameService.db.getCollection('user').insertOne({ userId, username, point: 0, coin: 0, });
  }

  // game current index
  async currentIndex(): Promise<number> {
    let data = await GameService.db.getCollection('reward').find({}).sort({ index: -1, }).limit(1).toArray();
    return data[0].index;
  }
  ...
  // upvote
  async upvote(index: number, userId: string, upvoterId: string, type: 'point' | 'coin', cast: number, count: number, time: Date, ): Promise<void> {
    await GameService.db.getCollection('upvote').insertOne({ index, userId, upvoterId, type, count, time, });
    await GameService.db.getCollection('list').updateOne({ index, userId }, { $inc: { count, } });

    if (type === 'point') {
      await GameService.db.getCollection('user').updateOne({ userId: upvoterId, }, { $inc: { point: -cast, } });
    }
  ...
  async addPoint(userId: string, point: number, coin: number, time: Date, ) {
    await GameService.db.getCollection('user').updateOne({ userId, }, { $inc: { point, coin, }, });
  ...
  async myUpvote(index: number, userId: string): Promise<any> {
    ...
    let data = await GameService.db.getCollection('upvote').aggregate(pipes);
    return data;
  }
